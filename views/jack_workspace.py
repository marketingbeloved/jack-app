...
def _launch_bg_generation(req: dict, t: str) -> None:
    ...
    st.session_state["ws_messages"].append({"who": "jack", "text": (
        "🐾 Принял — думаю над скриптом в фоне (несколько углов → выбираю сильнейший). "
        "Появится здесь сам через ~2–3 мин. Можно закрыть или обновить вкладку — не потеряется."), "time": t})
